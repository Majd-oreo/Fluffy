    ...
    <div class="col-md-6">
        <div class="card shadow" style="border: 2px solid #FF5B2E; background-color: white;">
            ...
            <div class="card-body">
                ...
                @endif
                <form action="{{ isset($pet) ? route('employee.pets.update', $pet->id) : route('employee.pets.store') }}" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()">
                    @csrf
                    @if(isset($pet))
                        @method('PUT')
                    @endif
